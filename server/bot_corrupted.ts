...
class TeleShopBot {
      ...
      switch (data) {
        ...
        default:
          ...
          // Handle quantity selection
            ...
            await this.handleQuantitySelection(chatId, query.from.id.toString(), productId, quantity);
          }
  ...
  private async handleProductDetails(chatId: number, userId: string, productId: string) {
    ...
    if (product.stock > 0) {
      ...
      actionButtons.push([
        ...
        { text: '✏️ Custom Qty', callback_data: `custom_qty_${productId}` }
      ]);
  ...
  private async handleAddToWishlist(chatId: number, userId: string, productId: string) {
    ...
    // For now, just show success message and return to main menu
